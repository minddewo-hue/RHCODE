...
Edit `gateway.config.json` so every public model maps to a real provider and upstream model. Then replace all placeholder values in `.env`. Never put a key directly in JSON; `api_key` is rejected and `api_key_env` must reference a non-empty environment variable.
...
The launcher uses `transfer/.codex-test` as `CODEX_HOME`, rebuilds the private model catalog,
